...
#[derive(Debug)]
pub struct ICSCanBase<'a,M,F,I,CLK> 
where 
    M: ErrMap,
    F: Frame,
...
{
    ics: ICS<'a,M,ICS_PAYLOAD_SIZE,u8,I,CLK>,
    can_id: Id,
    send_f: SendCanFun<F>,
}

impl<'a,M: ErrMap,F:Frame,I,CLK> ICSCanBase<'a,M,F,I,CLK>
where
    I: embedded_timers::instant::Instant,
    CLK: embedded_timers::clock::Clock<Instant = I>,
{
    pub fn new(ics_can_id: u16, ics_internal_id: u8, send_f: SendCanFun<F>,clock: CLK) -> Self
    {
        let ics : ICS<'a,M,ICS_PAYLOAD_SIZE,u8,I,CLK> = ICS::new(ics_internal_id,clock).ok().unwrap();
    ...
    {
        let fc = |ics: &mut ICS<M,ICS_PAYLOAD_SIZE,u8,I,CLK>, ics_mex: &ICSMex<ICS_PAYLOAD_SIZE,u8,u8>|
        {
    ...
    where FUN: FnMut (&mut ICS<M,ICS_PAYLOAD_SIZE,u8,I,CLK>,&ICSMex<ICS_PAYLOAD_SIZE,u8,u8>) -> (),
    {
