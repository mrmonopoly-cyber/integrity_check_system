...
impl<'a,M: ErrMap> ICSCan<'a,M>
{
    ...
    pub fn check_mex_general(&mut self, mex: &frame::Frame) -> Result<(),()>{
        if mex.id() == self.can_id{
            ...
            self.ics.check_general_mex(&ics_mex);
            Ok(())
        }else{
            Err(())
        }
    ...
    pub fn check_mex_specific_err(&mut self, mex: &frame::Frame,err_index:usize) -> Result<(),()>{
        if mex.id() == self.can_id{
            let mex_data = mex.data();
            let id_part = mex_data[0];
            let id = id_part & 0xF0;
            ...
            let ics_mex = ICSMex::new(id, part, err_vec);
            let _ = self.ics.check_specific_mex(&ics_mex,err_index);
            Ok(())
        }else{
            Err(())
        }
    ...
    pub fn run_check(&mut self){
        ...
        let errs : crate::ics_trait::ics_mex::ICSMexFull<7, u8, IcsPartType> =
